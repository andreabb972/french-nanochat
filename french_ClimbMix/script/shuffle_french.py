...
INPUT_DIR = os.environ.get('INPUT_DIR', os.path.expanduser("~/.cache/huggingface/hub/datasets--manu--french-30b/snapshots/6bdeaf311e673575f0f7d4c7f90d7ffbd879729b/data"))
OUTPUT_DIR = os.environ.get('OUTPUT_DIR', os.path.expanduser("~/.cache/huggingface/hub/datasets--manu--french-30b-shuffled/data"))
...
def clean_and_cast_french(table):
    """ Force l'ordre des colonnes, nettoie les id parasites et passe en large_string """
    ...
    for i, field in enumerate(table.schema):
        ...
        if field.name == 'dataset_id':
            col = pa.array(['french'] * len(table), type=pa.large_string())
...
print(f"Nombre de fichiers d'entrée trouvés: {len(files)}", flush=True)

for i, f in enumerate(files):
    table = pq.read_table(f)
    table = clean_and_cast_french(table)
    n = len(table)
...
for i, path in enumerate(output_paths):
    print(f"  [{i+1}/{N_OUTPUT}] {os.path.basename(path)}: lecture et permutation...", flush=True)
...
print("\nDone! Le dataset français est prêt.", flush=True)
