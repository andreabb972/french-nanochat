"""
Supervised fine-tuning (SFT) the model with a combined and weighted dataset (SmolTalk Included).
Run as:

python -m scripts.Full_chat_sft

Or torchrun for training:

torchrun --standalone --nproc_per_node=1 -m scripts.Full_chat_sft -- --device-batch-size=16
"""
...
# -----------------------------------------------------------------------------
# FIXED SECTION: SFT data mixture with SmolTalk AND doubled Croissant weight
# -----------------------------------------------------------------------------
croissant_filepath = "/home/abinetru/French_sft/data/croissant_nanochat.jsonl"
identity_conversations_filepath = os.path.join(base_dir, "identity_conversations.jsonl")

train_tasks = [
    SmolTalk(split="train"),
    CustomJSON(filepath=croissant_filepath),                     # Restauration du dataset général de discussion
    CustomJSON(filepath=croissant_filepath),     # Instance Croissant 1
...
print0(f"Training mixture: {len(train_dataset):,} rows (SmolTalk + Croissant + Identity x2 + MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs})")
...
# -----------------------------------------------------------------------------
# Data Loader and generator logic
last_step = False 
approx_progress = 0.0 
current_epoch = 1 
